...
public class OnboardingRepository(AdminDbContext context) : IOnboardingRepository
{
    ...
    public Task<bool> IsInvitedEmailInUseAsync(
        string normalizedEmail,
        Guid onboardingUserId,
        CancellationToken cancellationToken)
    {
        return context.OnboardingInvitedUsers.AnyAsync(
            x => x.NormalizedEmail == normalizedEmail && x.OnboardingUserId != onboardingUserId,
            cancellationToken);
    }

    public async Task<IReadOnlyCollection<string>> GetEmailsInUseAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var invitedEmails = await context.OnboardingInvitedUsers
            .Where(x => x.OnboardingUserId != onboardingUserId &&
                        normalizedEmails.Contains(x.NormalizedEmail))
            .Select(x => x.NormalizedEmail)
            .ToArrayAsync(cancellationToken);
