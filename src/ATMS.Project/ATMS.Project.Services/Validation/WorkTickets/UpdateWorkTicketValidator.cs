using ATMS.Project.Contracts.Commands.WorkTickets;
using ATMS.Project.Data.Repositories.Interfaces;
using ATMS.Project.Services.Resources;
using FluentValidation;

namespace ATMS.Project.Services.Validation.WorkTickets;

public class UpdateWorkTicketValidator : AbstractValidator<UpdateWorkTicketCommand>
{
    public UpdateWorkTicketValidator(
        IWorkTicketRepository workTicketRepository,
        IDictionariesRepository dictionariesRepository)
    {
        RuleFor(command => command.ProjectId)
            .NotEmpty().WithMessage(WorkTicketMessages.ProjectRequired);

        RuleFor(command => command.WorkTicketId)
            .NotEmpty().WithMessage(WorkTicketMessages.TicketRequired);

        RuleFor(command => command.WorkTicketStatusId).Cascade(CascadeMode.Stop)
            .NotEmpty().WithMessage(WorkTicketMessages.StatusRequired)
            .MustAsync(dictionariesRepository.IsWorkTicketStatusExistAsync)
            .WithMessage(WorkTicketMessages.StatusUnsupported);

        RuleFor(command => command)
            .SetValidator(new WorkTicketValidator(workTicketRepository, dictionariesRepository));
    }
}
