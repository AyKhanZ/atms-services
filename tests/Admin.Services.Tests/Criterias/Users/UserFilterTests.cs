...
namespace Admin.Services.Tests.Criterias.Users;
...
public class UserFilterTests
{
    ...
    [Fact]
    public void Apply_FilterByName_ReturnsMatchingUsers()
    {
        var filter = new UserFilter { Name = "Ivan" };
    ...
    [Fact]
    public void Apply_FilterByName_CaseInsensitive()
    {
        var filter = new UserFilter { Name = "ivan" };
        var result = filter.Apply(GetQuery()).ToList();
        Assert.Single(result);
    ...
    [Fact]
    public void Apply_FilterByName_IgnoresLeadingSpaces()
    {
        var filter = new UserFilter { Name = "  Ivan" };
        var result = filter.Apply(GetQuery()).ToList();
        Assert.Single(result);
    ...
    [Fact]
    public void Apply_FilterByName_EmptyString_ReturnsAll()
    {
        var filter = new UserFilter { Name = "" };
        var result = filter.Apply(GetQuery()).ToList();
    ...
    [Fact]
    public void Apply_FilterBySurname_ReturnsMatchingUsers()
    {
        var filter = new UserFilter { Surname = "Petrov" };
        var result = filter.Apply(GetQuery()).ToList();
        Assert.Equal(2, result.Count); // Petrov + Petrova
    }
 
    [Fact]
    public void Apply_FilterBySurname_CaseInsensitive()
    {
        var filter = new UserFilter { Surname = "petrov" };
        var result = filter.Apply(GetQuery()).ToList();
    ...
    [Fact]
    public void Apply_FilterByEmail_ReturnsMatchingUsers()
    {
        var filter = new UserFilter { Email = "ivan@mail" };
        var result = filter.Apply(GetQuery()).ToList();
        Assert.Single(result);
    }
 
    [Fact]
    public void Apply_FilterByEmail_CaseInsensitive()
    {
        var filter = new UserFilter { Email = "IVAN@MAIL" };
        var result = filter.Apply(GetQuery()).ToList();
        Assert.Single(result);
    ...
    [Fact]
    public void Apply_NameAndStatus_ReturnsCombinedResult()
    {
        var filter = new UserFilter { Name = "Iva", UserStatusId = 1 };
        var result = filter.Apply(GetQuery()).ToList();
        Assert.Single(result);
        Assert.Equal("Ivan", result[0].Name);
    }
    ...
    [Fact]
    public void PaginationCriteria_PageBelowOne_DefaultsToPage1()
    {
        var pagination = new PaginationCriteria<User>(0, 2);
        Assert.Equal(1, pagination.Page);
    }
 
    [Fact]
    public void PaginationCriteria_PageSizeAboveMax_ClampsToMax()
    {
        var pagination = new PaginationCriteria<User>(1, 999);
