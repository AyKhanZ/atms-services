...
namespace Project.Services.Tests.Criteria.Organizations;
...
public class OrganizationFilterTest
{
    ...
    [Fact]
    public void Apply_WhenSearchProvided_ShouldAddSearchCriteria()
    {
        var filter = new OrganizationFilter { Search = "Alpha" };
        var result = filter.Apply(GetSampleData());
        Assert.Contains("ILike", result.Expression.ToString());
    }

    [Fact]
    public void Apply_WhenSearchMatchesVoen_ShouldAddSearchCriteria()
    {
        var filter = new OrganizationFilter { Search = "222" };
        var result = filter.Apply(GetSampleData());
        Assert.Contains("ILike", result.Expression.ToString());
    }
    ...
    [Fact]
    public void Apply_WhenSortByCreatedAtAsc_ShouldReturnOrderedByCreatedAt()
    {
        var filter = new OrganizationFilter { SortBy = "createdAt", SortDirection = SortDirectionEnum.Asc };
        var result = filter.Apply(GetSampleData()).ToList();
    ...
    [Fact]
    public void Apply_WhenSearchAndDateRangeCombined_ShouldAddBothCriteria()
    {
        var filter = new OrganizationFilter
        {
            Search = "Alpha",
        ...
        Assert.Contains("CreatedAt", expression);
    }
